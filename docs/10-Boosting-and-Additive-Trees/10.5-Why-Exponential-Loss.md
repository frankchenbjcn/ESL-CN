...
$$
因此，由 AdaBoost 得到的加性展开是在估计 $P(Y=1\mid x)$ **对数几率 (log-odds)** 的二分之一．这证明使用它的符号作为 \eqref{10.1} 的分类规则是合理的．
...
另外一个有着同样的总体最小化的损失准则为 **二项分布负对数似然 (binomial negative log-likelihood)** 或者 **偏差 (deviance)**（也称为交叉熵），将 $f$ 解释为 logit 变换．
